...
class Player(GameObject):
    def __init__(self):
        ...
        for j in range(1, 7):
            self.jump_img.append(load_img('./trem/jump%d.png' % j))
    ...
    def jump(self):
        ...
        if self.y > INIT_Y_P - 220 and not self.down_flag:
            ...
            self.img = self.jump_img[2]
        else:
            self.down_flag = True
        if self.down_flag and self.y < INIT_Y_P:
            self.y += self.jump_speed
            self.img = self.jump_img[4]
            if self.y >= INIT_Y_P:
                self.down_flag = False
                is_jump = False

        # if self.jump_index >= len(self.jump_img):
        #     self.jump_index = 0
        #     self.y = INIT_Y_P
        #     is_jump = False
        # self.img = self.jump_img[self.jump_index]
        # if 0 <= self.jump_index <= 2:
        #     self.y -= self.jump_speed
        #     # print('up', self.y)
        # elif 3 <= self.jump_index <= 5:
        #     self.y += self.jump_speed
        #     if self.y > INIT_Y_P:
        #         self.y = INIT_Y_P
        #     # print('down', self.y)
...
class MySprite(pygame.sprite.Sprite):
    ...
    def load(self, filename, width, height, columns):
        self.master_image = load_img(filename).convert_alpha()
        self.frame_width = width
        self.frame_height = height
        self.rect = 350, 270, width, height
        self.columns = columns
        rect = self.master_image.get_rect()
        # print('rect in load is%s' % rect)
